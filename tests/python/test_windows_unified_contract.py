...
class WindowsUnifiedContractTests(unittest.TestCase):
    ...
    def test_builder_is_source_addressed_and_fail_closed(self) -> None:
        ...
        self.assertIn('"--target-profile"', builder)
        self.assertIn(
        ...
        self.assertNotIn('"CHAINLOADING=1"', builder)
        self.assertIn('"ram_chainload_target": True', builder)
        self.assertIn('"paired-1TR recoveryOS"', builder)
        self.assertIn('"lowest_virtual_address": 0', builder)
    ...
    def test_j813_seal_describes_only_the_m5_air_target(self) -> None:
        module = runpy.run_path(
            str(ROOT / "tools/build-j414s-windows-unified.py")
        )
        profile = module["target_profile"]("j813")
        self.assertEqual(profile["target"]["board"], "J813")
        self.assertEqual(profile["target"]["soc"], "T8142")
        self.assertEqual(profile["target"]["compatible"], "apple,j813")
        self.assertEqual(profile["internal_nvme"]["m1n1_access"], "read-only")
        self.assertEqual(profile["internal_nvme"]["efi_block_io_owner"], "Mu ANS DXE")
        self.assertFalse(profile["warm_usb_reenumeration_validated"])
        self.assertNotIn("xhc2_right_usb_c", profile)
        self.assertNotIn("wireless_descriptor", profile)

        required = module["REQUIRED_BINARY_SYMBOLS"]
        for symbol in (
            "chainload_load",
            "fb_init",
            "hv_map_vuart",
        ...
        self.assertIn(
            "T8142", module["TARGET_REQUIRED_BINARY_STRINGS"]["j813"]
        )

    def test_rust_archive_tracks_cargo_feature_flags(self) -> None:
        makefile = (ROOT / "Makefile").read_text(encoding="utf-8")
        self.assertIn(
            "RUST_FLAGS_STAMP := $(BUILD_DIR)/.rust-cargo-flags", makefile
        )
        self.assertIn("$(RUST_FLAGS_STAMP): FORCE", makefile)
        self.assertIn(
            "$(BUILD_DIR)/$(RUST_LIB): $(RUST_FLAGS_STAMP)", makefile
        )
    ...
    def test_xhc2_typec_policy_is_bounded_and_does_not_touch_proxy(self) -> None:
        ...
        self.assertIn("tps6598x_wait_plug_settled", tps)
        self.assertIn("TPS6598X_HOST_PORT_EMPTY", tps)
        self.assertNotIn(
            "i2c_smbus_write(dev->i2c, dev->addr, TPS_REG_SYSTEM_CONFIG", tps
        )
