...
class HV(Reloadable):
    ...
    def __init__(self, iface, proxy, utils):
        ...
        self._bp_hmc = [False, False, False, False, False]
        self._bp_hooks = dict()
    ...
    def handle_step(self, ctx):
        ...
        for i, vaddr in enumerate(self._bps):
            ...
            self.u.msr(
                DBGBCRn_EL1(i),
                DBGBCR(E=1, PMC=0b11, HMC=self._bp_hmc[i], BAS=0xf).value,
            )
    ...
    def handle_break(self, ctx):
        ...
        # EL2 firmware cannot use the normal single-step re-arm path: the
        # resulting debug exception is taken by the resident monitor itself.
        # Treat higher-EL breakpoints as one-shot and re-arm the other slots
        # before returning to the trapped instruction.
        if ctx.elr in self._bps:
            idx = self._bps.index(ctx.elr)
            if self._bp_hmc[idx]:
                self._bps[idx] = None
                self._bp_hmc[idx] = False
                self._bp_hooks.pop(ctx.elr, None)
    ...
    def add_hw_bp(self, vaddr, hook=None, higher_el=False):
        if None not in self._bps:
            raise ValueError("Cannot add more HW breakpoints")

        i = self._bps.index(None)
        control = DBGBCR(E=1, PMC=0b11, HMC=higher_el, BAS=0xf).value
        if self.ctx is None:
            # Pre-launch scripts run before hv.start() creates a guest context.
            # Arm the current boot CPU directly; secondary CPUs do not exist yet.
            self.u.msr(DBGBCRn_EL1(i), control)
    ...
    def add_sym_bp(self, name, hook=None, higher_el=False):
        return self.add_hw_bp(
    ...
    def init(self):
        ...
        mdcr = MDCR(0)
        # Apple does not expose the architectural PMUv3 register bank directly.
        # Trap both PMCR_EL0 and the remaining PMU register accesses so the C
        # hypervisor's SYS_PM* handlers can present the standard interface that
        # Windows expects instead of taking an EL1 undefined-instruction abort.
        mdcr.TPMCR = 1
        mdcr.TPM = 1
        mdcr.TDE = 1
        mdcr.TDA = 1
    ...
    def start(self):
        ...
        # hypervisor setup.  Reprogram the debug registers here, on the exact
        # physical CPU that is about to enter the guest, so a breakpoint cannot
        # be lost to setup-time register state or a boot-CPU handoff.
        for i, vaddr in enumerate(self._bps):
            if vaddr is None:
                continue
            self.u.msr(DBGBVRn_EL1(i), vaddr)
