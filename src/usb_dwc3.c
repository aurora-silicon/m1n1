...
#ifdef ENABLE_USB_RXDIAG
#define usb_rxdiag_printf(fmt, ...) debug_printf("usb-dwc3@%lx: " fmt, dev->regs, ##__VA_ARGS__)
#else
#define usb_rxdiag_printf(fmt, ...)                                                                 \
    do {                                                                                            \
    } while (0)
#endif
                                                ...
                                                const union usb_setup_packet *setup)
{
    switch (setup->raw.bRequest) {
        ...
        case USB_REQUEST_SET_CONFIGURATION:
            usb_rxdiag_printf("USB RXDIAG: configuration=%u\n", setup->set_configuration.configuration);
        ...
        case USB_REQUEST_GET_CONFIGURATION:
            usb_rxdiag_printf("USB RXDIAG: get-configuration=%u\n", dev->configuration);
        ...
        default:
            usb_dwc3_ep_set_stall(dev, 0, 1);
            dev->ep0_state = USB_DWC3_EP0_STATE_IDLE;
            usb_rxdiag_printf("USB RXDIAG: unsupported device SETUP type=%02x req=%02x value=%04x "
                                                   ...
                                                   const union usb_setup_packet *setup)
{
    switch (setup->raw.bRequest) {
        ...
        default:
            usb_dwc3_ep_set_stall(dev, 0, 1);
            dev->ep0_state = USB_DWC3_EP0_STATE_IDLE;
            usb_rxdiag_printf("USB RXDIAG: unsupported interface SETUP type=%02x req=%02x value=%04x "
                   "index=%04x length=%04x\n",
                   setup->raw.bmRequestType, setup->raw.bRequest, setup->raw.wValue,
                   setup->raw.wIndex, setup->raw.wLength);
    }
                                                  ...
                                                  const union usb_setup_packet *setup)
{
    switch (setup->raw.bRequest) {
        ...
        default:
            usb_dwc3_ep_set_stall(dev, 0, 1);
            dev->ep0_state = USB_DWC3_EP0_STATE_IDLE;
            usb_rxdiag_printf("USB RXDIAG: unsupported endpoint SETUP type=%02x req=%02x value=%04x "
                   "index=%04x length=%04x\n",
                   setup->raw.bmRequestType, setup->raw.bRequest, setup->raw.wValue,
                   setup->raw.wIndex, setup->raw.wLength);
    }
...
static void usb_dwc3_ep0_handle_class(dwc3_dev_t *dev, const union usb_setup_packet *setup)
{
    ...
    switch (setup->raw.bRequest) {
        ...
        case USB_REQUEST_CDC_SET_CTRL_LINE_STATE:
            usb_rxdiag_printf("USB RXDIAG: ctrl-line pipe=%d value=%04x ep-out=%u\n", pipe,
...
static void usb_dwc3_cdc_start_bulk_out_xfer(dwc3_dev_t *dev, u8 endpoint_number)
{
    ...
    if (ret) {
        usb_rxdiag_printf("USB RXDIAG: arm OUT ep=%u failed=%d\n", endpoint_number, ret);
        return;
    }

    if (!usb_rxdiag_out_armed[endpoint_number]) {
        usb_rxdiag_printf("USB RXDIAG: armed OUT ep=%u size=%u\n", endpoint_number, XFER_SIZE);
                                                   ...
                                                   const struct dwc3_event_depevt event)
{
    ...
    usb_rxdiag_printf("USB RXDIAG: OUT complete ep=%u bytes=%lu residual=%u\n", event.endpoint_number,
...
static void usb_dwc3_handle_event_ep(dwc3_dev_t *dev, const struct dwc3_event_depevt event)
{
    if ((event.endpoint_number == USB_LEP_CDC_BULK_OUT ||
        ...
        usb_rxdiag_printf("USB RXDIAG: event ep=%u type=%u status=%u params=%u\n", event.endpoint_number,
